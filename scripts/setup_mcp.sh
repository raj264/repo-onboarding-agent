...
if [ -n "$DESKTOP_CONFIG" ]; then
    ...
    "$VENV_PYTHON" - "$DESKTOP_CONFIG" "$VENV_PYTHON" "$TARGET_REPO_ABS" "$ALLOW_PR" <<'PYEOF'
import json
import pathlib
import sys

config_path, venv_python, target_repo, allow_pr = sys.argv[1:5]
...
config["mcpServers"]["repo-onboarding-agent"] = {"command": venv_python, "args": args}
...
if command -v claude >/dev/null 2>&1; then
    CMD=(claude mcp add repo-onboarding-agent -- "$VENV_PYTHON" -m onboarding_agent.mcp_server --target-repo "$TARGET_REPO_ABS")
